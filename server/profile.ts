...
import { CATEGORIES, type Category, type Product, type Profile } from "../shared/decision.ts";
...
export function buildProfile(userId: string, products: Product[]): Profile {
  ...
  const affinity = Object.fromEntries(CATEGORIES.map((c) => [c, 0])) as Record<Category, number>;
  ...
  for (const c of CATEGORIES) affinity[c] = Math.max(0, +(affinity[c] / max).toFixed(2));
