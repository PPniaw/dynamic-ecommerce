...
- **第一次來的訪客先引導**(demo 的主軸是「先認識你 → 店長成你的樣子 → 再找商品」):預設顧客 `u_new` 叫「你」,
  **每個瀏覽器第一次來就建立自己的「你」**(`POST /api/users`,id 記在 localStorage `llm-shop:visitor`,資料全空);
  面板只列 4 位示範顧客(`DEMO_SHOPPER_IDS`)+ 自己的「你」,看不到別人的。`u_new` 只是建立失敗時的備援。
  還沒親口說過任何特質(`nothingStated`,猜的不算)時跳歡迎卡(`Welcome.tsx`)→「聊聊」或「直接選特質」。
