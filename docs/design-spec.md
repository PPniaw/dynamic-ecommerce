...
| 版面(LLM) | 每次行為 / 聊天 | **粒子重組**(2026-09-25 取代 View Transition):畫面上的文字、圖片、色塊變成粒子,飛到新版面的位置再淡入;通用,不綁元件 |
...
- 聊天(2026-09-25):顧客的話 → 偏好更新(enum)→ 立刻重排;回話可以由 Claude 寫(唯一的 LLM 文字),
  但不准提價格 / 庫存,商品卡由程式顯示。發布的預覽頁用 artifact 的 `sample`,走觀看者自己的 Claude 訂閱
- 從行為猜個性(2026-09-25):Jev + 規則加權,猜的標成「猜」、可以點掉(點掉就不再猜),
  顧客自己說的永遠優先;只猜點擊看得出來的 10 種(不猜內向 / 外向 / 感性 / 理性)
